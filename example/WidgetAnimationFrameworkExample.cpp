...
public:
      ...
      : QFrame(_parent)
  {
    ...
    QObject::connect(
        close, &QToolButton::clicked, this, &NotifyMessage::hideMessage);
        ...
        [=]
        {
          if (m_lastHeight != maximumHeight()) {
            ...
            parentWidget()->resize(parentWidget()->width(),
                                   parentWidget()->height() + delta);
            parentWidget()->move(parentWidget()->pos().x(),
                                 parentWidget()->pos().y() - delta);
          }
        });
    timer->start(10);
  }
...
int main(int argc, char* argv[])
{
  ...
  QLabel* mainToolbarTitle =
      new QLabel("<b>Widgets Animation Framework demo</b>", mainToolbar);
  ...
  textEdit->setHtml(
      ...
      "<p>Slide widget from side of application and make background little "
      "darker for accent attension to slided widget. To do it just call</p>"
      "<p><pre>WAF::Animation::sideSlide</pre></p>"
      "<p>with your cool widget for slide and set application side from which "
      "their must slide.</p>"
  ...
  NotifyMessage* connectionMessage =
      new NotifyMessage("Connections estabilished", notify);
  NotifyMessage* subscriptionMessage = new NotifyMessage(
      "Subscribe to <b>pro</b> account and get more available "
      "features.<br/><br/><a href=\"http://dimkanovikov.pro\">Read more</a>",
      notify);
  ...
  // Настроим соединения
  //
  QObject::connect(mainToolbarMenuButton,
                   &QToolButton::clicked,
                   [=] { WAF::Animation::sideSlideIn(menu, WAF::LeftSide); });
  QObject::connect(menuToolbarBackButton,
                   &QToolButton::clicked,
                   [=] { WAF::Animation::sideSlideOut(menu, WAF::LeftSide); });
  QObject::connect(menuButtonLogin,
                   &QPushButton::clicked,
                   [=]
                   {
  ...
  QObject::connect(authLoginButton,
                   &QPushButton::clicked,
                   [=]
                   {
  ...
  QObject::connect(authCancelButton,
                   &QPushButton::clicked,
                   [=] { WAF::Animation::sideSlideOut(auth, WAF::TopSide); });
  QObject::connect(
      menuButtonNotify,
  ...
  QObject::connect(tabs,
                   ...
                   [=](int _showWidgetIndex)
                   {
                     WAF::StackedWidgetAnimation::fadeIn(
                         pages, pages->widget(_showWidgetIndex));
                   });

  //
  // Скрываем или показываем тулбар и вкладки, в зависимости от направления
  // прокрутки текстового редактора
  //
  QObject::connect(
  ...
  QObject::connect(creditsInfo,
                   &QPushButton::clicked,
                   [=]
                   {
                     static int i = 2;
                     WAF::Animation::circleFill(creditsInfo,
  ...
  w.setStyleSheet(
      ...
      "QToolButton { border: none; min-width: 40px; min-height: 40px; "
      "icon-size: 24px; }"
      "QTextEdit { border: none; }"
      "QPushButton[menu=true] { text-align: left; background-color: white; "
      "border: none; border-bottom: 1px solid palette(dark); padding: 8px; }"
      "QFrame[toolbar=true] { background-color: #66C966; }"
      "QFrame[menu=false] { background-color: palette(window); }"
      "QFrame[menu=true] { background-color: white; border: none; "
  ...
  auto simpleUsage = []()
  {
    ...
    const auto layMain = new QVBoxLayout(w);
    const auto btnShw = new QPushButton("Show", w);
    const auto btnHide = new QPushButton("Hide", w);

    layMain->addWidget(btnShw);
    const auto c = new QWidget(w);
    c->setVisible(false);
    layMain->addWidget(c);
    //        c->setVisible(false);
    const auto lay = new QVBoxLayout(c);
    c->setLayout(lay);
    for (auto i = 0; i < 10; i++)
      lay->addWidget(new QPushButton(QString("%1").arg(i), c));

    QObject::connect(btnHide,
                     &QAbstractButton::clicked,
                     [=] { WAF::Animation::sideSlideIn(c, WAF::BottomSide); });
    QObject::connect(btnShw,
                     &QAbstractButton::clicked,
                     [=] { WAF::Animation::sideSlideIn(c, WAF::TopSide); });
    w->show();
  };
