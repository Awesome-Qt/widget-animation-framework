...
void StackedWidgetAnimation::slide(QStackedWidget* _container,
                                   QWidget* _widget,
                                   WAF::AnimationDirection _direction)
{
  const StackedWidgetAnimationPrivate::AnimatorType animatorType =
      StackedWidgetAnimationPrivate::Slide;
  AbstractAnimator* animator = 0;
  if (pimpl()->hasAnimator(_widget, animatorType)) {
    animator = pimpl()->animator(_widget, animatorType);
    if (StackedWidgetSlideAnimator* slideAnimator =
            qobject_cast<StackedWidgetSlideAnimator*>(animator))
    {
      slideAnimator->setAnimationDirection(_direction);
    }
  } else {
    StackedWidgetSlideAnimator* slideAnimator =
        new StackedWidgetSlideAnimator(_container, _widget);
    slideAnimator->setAnimationDirection(_direction);
...
void StackedWidgetAnimation::slideOverIn(QStackedWidget* _container,
                                         QWidget* _widget,
                                         WAF::AnimationDirection _direction)
{
  const bool IN = true;
...
void StackedWidgetAnimation::slideOverOut(QStackedWidget* _container,
                                          QWidget* _widget,
                                          WAF::AnimationDirection _direction)
{
...
void StackedWidgetAnimation::slideOver(QStackedWidget* _container,
                                       QWidget* _widget,
                                       WAF::AnimationDirection _direction,
                                       bool _in)
{
  const StackedWidgetAnimationPrivate::AnimatorType animatorType =
      StackedWidgetAnimationPrivate::SlideOver;
  AbstractAnimator* animator = 0;
  if (pimpl()->hasAnimator(_widget, animatorType)) {
    animator = pimpl()->animator(_widget, animatorType);
    if (StackedWidgetSlideOverAnimator* slideOverAnimator =
            qobject_cast<StackedWidgetSlideOverAnimator*>(animator))
    {
      if (_in) {
    ...
    StackedWidgetSlideOverAnimator* slideOverAnimator =
        new StackedWidgetSlideOverAnimator(_container, _widget);
    slideOverAnimator->setAnimationDirection(_direction);
...
{
  const StackedWidgetAnimationPrivate::AnimatorType animatorType =
      StackedWidgetAnimationPrivate::FadeIn;
  AbstractAnimator* animator = 0;
  if (pimpl()->hasAnimator(_widget, animatorType)) {
    animator = pimpl()->animator(_widget, animatorType);
  } else {
    StackedWidgetFadeInAnimator* fadeInAnimator =
        new StackedWidgetFadeInAnimator(_container, _widget);
