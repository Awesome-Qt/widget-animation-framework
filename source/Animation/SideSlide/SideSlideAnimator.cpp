...
void SideSlideAnimator::slideIn()
{
  ...
  //
  // Позиционируем виджет для анимации в исходное положение и настраиваем его
  // размер
  //
...
bool SideSlideAnimator::eventFilter(QObject* _object, QEvent* _event)
{
      ...
      && _event->type() == QEvent::Resize)
  {
    ...
    switch (m_side) {
      case WAF::RightSide: {
        widgetForSlide()->move(
            widgetForSlideParent->width() - widgetForSlide()->width(), 0);
        //
      ...
      case WAF::BottomSide: {
        widgetForSlide()->move(
            0, widgetForSlideParent->height() - widgetForSlide()->height());
        //
